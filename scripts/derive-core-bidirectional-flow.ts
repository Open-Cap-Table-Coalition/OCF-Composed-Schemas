...
function renderInverseCoverageStory(
  ...
  return [
    ...
    `7. We have mapping evidence for **${story.mappedDefs}**: **${story.fullyMappedDefs}** fully mapped and **${story.partiallyMappedDefs}** partially mapped (**${counts.direct}** direct executable, **${counts["type-only"]}** type-only, **${counts.deferred}** deferred).`,
    `8. **${story.unmappedCandidateDefs}** standalone candidates have no mapping evidence yet; their inventory role tells us whether that is expected or actionable (**${counts["report-rollup"]}** report/read-model roll-ups, **${counts.alternate}** alternate shapes, **${counts["vendor-family"]}** CARTA-specific families without OCF sources, **${counts["workflow-gap"]}** workflow/data gaps, **${counts.gap}** actionable gaps, **${counts.review}** requiring review).`,
    "",
