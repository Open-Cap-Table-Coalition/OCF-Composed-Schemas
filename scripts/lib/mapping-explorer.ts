...
function filterBar(group: string): string {
  return `<div class="filter-controls" data-filter-group="${html(
    ...
    group
  )}"><div class="filter-buttons"><button class="filter-button is-active" data-filter-button="all" type="button">All</button><button class="filter-button" data-filter-button="mapped" type="button">Mapped</button><button class="filter-button" data-filter-button="gap" type="button">Gaps</button>${
    group === "Carta targets"
      ? '<button class="filter-button" data-filter-button="support" type="button">Support</button>'
      : ""
  }</div></div>`;
}
...
export function renderMappingExplorerIndex(data: MappingExplorerData): string {
  ...
  return shell(
    ...
    <section id="ocf-objects" class="directory-section"><div class="section-heading"><div><span class="eyebrow">01 / source side</span><h2>OCF objects</h2><p>Each page keeps the authored mapping visible, including objects with no Carta target evidence.</p></div>${filterBar(
      ...
      .map(sourceCard)
      .join("")}</div></section>
    <section id="carta-targets" class="directory-section"><div class="section-heading"><div><span class="eyebrow">02 / target side</span><h2>Carta targets</h2><p>Every object-like target is browseable, including standalone candidates with no OCF source and nested support definitions.</p></div>${filterBar(
      "Carta targets"
    )}</div><div class="directory-grid" data-directory="Carta targets">${data.targets
      .map(targetCard)
      .join("")}</div></section>
...
export function renderMappingExplorerCss(): string {
  return [
    ...
    ".directory-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; } .card { padding: 20px; min-height: 210px; display: flex; flex-direction: column; } .card.is-gap { border-color: rgba(255,146,127,.27); background: linear-gradient(145deg, rgba(74,31,42,.46), rgba(17,26,44,.72)); } .card-top, .card-footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; }",
    ".card-title { display: block; font-size: 22px; letter-spacing: -.05em; text-decoration: none; margin: 17px 0 10px; } .card-title:hover { color: var(--mint); } .card-copy { color: var(--muted); font-size: 12px; margin: 0 0 18px; }",
    ...
    ".mini-chip, .target-token, .source-token { display: inline-flex; align-items: center; width: max-content; border: 1px solid var(--line); background: rgba(255,255,255,.035); border-radius: 999px; padding: 5px 8px; color: var(--muted); font-size: 11px; } .chip-row, .token-stack { display: flex; flex-wrap: wrap; gap: 6px; } .card-footer { margin-top: auto; padding-top: 18px; } .issue-link { color: var(--coral); }",
