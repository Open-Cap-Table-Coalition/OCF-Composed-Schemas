...
const MAPPING_EXPLORER_TITLE = "Analysis of Carta Core vs OCF v1.0";
...
function targetCard(target: ExplorerTarget): string {
  ...
  return `<article class="card directory-card${
    ...
    <div class="card-footer">${externalLink(issueUrl, issueLabel, "text-link issue-link")}${link(
    `targets/${target.slug}.html`,
    "Inspect target →"
  )}</div>
  </article>`;
}
...
export function renderMappingExplorerIndex(data: MappingExplorerData): string {
  ...
  return shell(
    "Overview",
    "",
    `<section class="hero"><div class="hero-copy"><span class="eyebrow accent">Open Cap Table Coalition / member review</span><h1>Analysis of Carta Core vs OCF v1.0</h1><p class="hero-lede">Use this analysis to identify potential mapping gaps and help shape the resulting standard. Members can comment on specific fields or objects they need — or do not think should be included. Every page below includes a link to open a prefilled GitHub issue.</p><div class="hero-actions"><a class="button button-primary" href="#ocf-objects">Browse the analysis</a><a class="button button-quiet" href="assets/mapping-flows-interactive/index.html">Open interactive viewer ↗</a></div></div><div class="hero-orbit"><div class="orbit-ring ring-one"></div><div class="orbit-ring ring-two"></div><div class="orbit-core"><span class="orbit-mark" role="img" aria-label="Open Cap Table Coalition"></span><span>member</span><strong>review</strong><span>space</span></div><span class="orbit-tag tag-one">OCF</span><span class="orbit-tag tag-two">CARTA</span><span class="orbit-tag tag-three">GAPS</span></div></section>
    ...
    <section class="feature-grid"><div class="feature-copy"><span class="eyebrow">How to participate</span><h2>Review the evidence. Shape the standard.</h2><p>Explore the OCF objects and Carta targets below to compare Carta Core with OCF v1.0. Use the issue link on any page to comment on the fields or objects members need — or that should not be in the resulting standard.</p><div class="feature-list"><span><i class="dot dot-mint"></i>${html(
      data.metrics.sourceObjects
