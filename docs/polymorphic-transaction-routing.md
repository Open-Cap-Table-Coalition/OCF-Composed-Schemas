...
- `shared:` — fields whose mapping is identical across variants; expanded into every variant before coverage counting.
...
shared:
  ...
  security_id:               { kind: rename, target: "#/$defs/OptionGrant/properties/securityId" }   # [1]
...
# Notes
# [1] shared: security/identity fields are pinned to OptionGrant as a representative; the importer
#     writes the resolved family's security object (OptionGrant / RestrictedStockUnit). See §6.2.
...
#     variant). The §6.2 "shared: templating" gap is separate: it concerns the SHARED identity fields landing
#     on a different security object per variant (OptionGrant vs RestrictedStockUnit), not this field.
```
...
Roughly ten mappings are affected. The convention is purely additive, so adoption is incremental and each file migrates independently.
...
| **`shared:` target templating** | Shared identity fields land on a security object whose *type differs per variant* (`OptionGrant` vs `RestrictedStockUnit`). The example pins them to `OptionGrant`; a rigorous version needs a templated pointer (e.g. `#/$defs/{{security}}/properties/stakeholderId`) and validator expansion — deliberately out of scope. |
...
| Item | Status |
|---|---|
| **No SAR security `$def`** | Carta has the full SAR *transaction* family (`SarIssuanceTransaction`, `SarExerciseTransaction`, `SarCancellationTransaction`) but **no first-class SAR security object** like `OptionGrant`. The only SAR-level container is `SarTransactionItem` (a tx-item wrapper), which does carry `securityId`/`securityLabel`/`stakeholderId` — so identity fields *could* land there — but it exposes no slot for `custom_id`, `board_approval_date`, `vestings`, or `termination_exercise_windows`, which therefore have no home. **Do not invent a SAR security `$def`.** |
