...
- Raw JSON diff: 1,162 removed lines, 182 added lines, 30 hunks.
...
- `Certificate`: required identity/holder/issuer fields are now explicit; adds dividend accrual and returned-invested-capital fields.
...
- `Stakeholder`: requires `id`, `issuerId`, and `fullName`.
...
- [x] Mark removed summary/compliance/document targets explicitly as excluded or unmappable.
...
```text
node --loader ts-node/esm --no-warnings ./scripts/check-carta-schema-refresh.ts
node --loader ts-node/esm --no-warnings ./scripts/validate-mappings.ts
node --loader ts-node/esm --no-warnings ./scripts/generate-mapping-inverse-artifacts.ts --check
```

The checker verifies the pinned SHA/version, all 102 mapping versions, zero dangling Carta JSON pointers, zero pointers to the removed-definition set, explicit SAR/compliance/summary/document exclusions, and the generated Explainer link. It is intentionally separate from the general mapping validator so this refresh cannot silently pass with an incomplete migration.
