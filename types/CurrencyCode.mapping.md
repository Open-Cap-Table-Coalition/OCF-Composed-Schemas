...
- `CurrencyCode` is a bare scalar string type (zero properties), so there are no per-field rows; coverage is `0/0` and the correspondence lives at the type level (same convention as `types/Md5.mapping.md`).
