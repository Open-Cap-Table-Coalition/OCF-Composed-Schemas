...
- **OCF objects that `$ref` `Percentage` (routed at the object level, not here):**
  ...
  - `types/conversion_mechanisms/SharePriceBasedConversionMechanism.schema.json` → `discount_percentage` — no target in the effective warrant-right context; it must not be routed to `ConvertibleNote.discountPercentage`.
