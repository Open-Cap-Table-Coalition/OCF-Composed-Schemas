...
- `interest_rates` → `#/$defs/ConvertibleNote/properties/interestRate` (Carta `Decimal`). This is the array of `InterestRate` objects; consistent with `types/InterestRate.mapping.md`, each entry's `rate` lands on the note's flat `interestRate`. **Lossy in two ways:** (a) OCF allows an *array* of time-segmented rates (each with accrual `start`/`end` dates), but Carta stores a single scalar rate with no window dates, so a multi-rate step schedule collapses to one representative rate and the segment boundaries are dropped; (b) the per-segment accrual `start`/`end` dates on each `InterestRate` have no Carta home (see `InterestRate.mapping.md`). Representation matches at the value level: OCF stores the rate as a decimal fraction (e.g. `0.05`) and Carta's `Decimal` is the same fraction in string form — no scaling. Modeled as `rename` (Carta inlines as a bare scalar; OCF wraps it in an array of objects); the importer should pick the applicable/first segment and flag the dropped schedule.
