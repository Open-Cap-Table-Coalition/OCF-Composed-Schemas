...
fields:
  ...
  amount:
    kind: rename
    target:
      - "#/$defs/ConvertibleCancellationTransaction/properties/principal"
...
- `amount` → `principal` and `ConvertibleNote.cashPaid` (rename). OCF `amount` is `types/Monetary.schema.json` and both Carta fields are `#/$defs/Money`; the same amount is the cash paid to cancel the note and the principal recorded on the cancellation transaction. For a full cancellation these coincide with the note principal; for a partial cancellation the value is a cancellation delta, so the principal interpretation remains approximate.
