...
- `security_id` → `WarrantTransactionItem.securityId` (parent routing key). Carta's `WarrantCancellationTransaction` carries no security/warrant reference field itself, but the cancellation is structurally contained by the parent `WarrantTransactionItem`, whose `securityId` identifies the warrant and whose `cancellations[]` array holds the event. The OCF cancellation is therefore grouped into the parent by `security_id`, just like issuance, exercise, and transfer; the target path is the parent identity slot rather than the child transaction object.
