...
- `date`: OCF's transaction date (the conversion event date) maps to both `ConvertibleCancellationTransaction.effectiveDatetime` and the parent `ConvertibleNote.conversionDatetime`. **Granularity note:** OCF `date` is a calendar `Date` (`YYYY-MM-DD`); Carta's datetime fields are `Iso8601CompleteCalendarDateTime`, so an arbitrary/zero time component must be synthesized on export.
