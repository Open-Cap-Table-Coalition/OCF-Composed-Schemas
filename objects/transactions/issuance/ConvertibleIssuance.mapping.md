...
- `investment_amount`: OCF's `Monetary` principal invested and outstanding at issuance → both `ConvertibleIssuanceTransaction.principal` and the note-level `ConvertibleNote.cashPaid` (`Money`). The transaction copy records issuance principal; the note copy records the cash paid for the note. Both sides are amount-with-currency, so the OCF `Monetary {amount, currency}` maps field-for-field onto Carta `Money`. Cancellation `amount` remains scoped to `ConvertibleCancellationTransaction.principal` because it is a cancellation delta, not the note's original cash-paid value.
